...
router.post('/upload/remote', auth.protectApi, async (req, res) => {
    try {
        ...
        const filename = path.basename(url) || `remote_${Date.now()}`;
...
router.post('/upload/chunk/init', auth.protectApi, async (req, res) => {
    ...
    const session = new UploadSession({ sessionId, owner: req.user.id, filename, totalSize });
    await session.save();
    res.json({ sessionId });
});
...
router.post('/upload/chunk/finalize', auth.protectApi, async (req, res) => {
    ...
    const newFile = new File({
        ...
        customAlias: `chunk_${Date.now()}_${session.filename}`,
...
router.put('/files/:id/meta', auth.protectApi, async (req, res) => {
    try {
        ...
        if (tags !== undefined) update.tags = tags.split(',').map(t => t.trim());
